...
/* wm_user should be 0x400 */
#ifndef WM_GET_CURSOR
#define WM_GET_CURSOR (WM_USER + 0x100)
#endif
...
#define TO_CHR_Y(y) ((((y)<0 ? (y)-font_height+1: (y))-offset_height) / font_height)
	...
	return 0;
	  ...
	  case WM_GET_CURSOR:
		  // returns column in hiword, row in loword
		  // adjunst column and row to 1 based, we can use
		  // 0 or -ve values as indicators of an error?
		  return ((*term).curs.x + 1) * 0x10000 + (*term).curs.y + 1;
